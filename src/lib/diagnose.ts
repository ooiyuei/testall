...
- 過去のテスト結果のトレンドを summary で明示する。改善中なら強みとして言及し、悪化中なら根本原因を診断して作戦を示す。
`;
...
type TrendMetrics = {
  ...
  fixedWeakUnits: string[]; // 同一単元で2回続けて低下している弱点
};

function computeTrend(
  pastTests: PastTestSummary[],
  currentSubject: string,
): TrendMetrics | null {
  if (pastTests.length < 2) return null;
  ...
  const deltaPct = newest.scorePct - oldest.scorePct;
  ...
  let trend: TrendMetrics["trend"] = "stable";
  if (deltaPct >= 5) trend = "improving";
  else if (deltaPct <= -5) trend = "declining";

  // 同一単元で2回連続低下 → 弱点固定化フラグ
  const fixedWeakUnits: string[] = [];
  if (sorted.length >= 2) {
    const unitScores: Record<string, number[]> = {};
    for (const t of sorted.slice(0, 3)) {
      for (const u of t.weakUnits ?? []) {
        if (!unitScores[u]) unitScores[u] = [];
        unitScores[u].push(t.scorePct);
      }
    }
    for (const [unit, scores] of Object.entries(unitScores)) {
      if (scores.length >= 2 && scores[0] <= scores[1]) {
        fixedWeakUnits.push(unit);
      }
    }
  }
  ...
  return {
    ...
    deltaPct,
    periodDays: daysDiff,
    sameSubjectOnly: sameSubject.length >= 2,
    fixedWeakUnits,
  };
}
...
function buildUserPrompt(input: TestInput): string {
  ...
  if (history?.pastTests && history.pastTests.length > 0) {
    ...
    // 構造化トレンドメトリクス
    const trendMetrics = computeTrend(history.pastTests, input.subject);
    if (trendMetrics) {
      const sign = trendMetrics.deltaPct > 0 ? "+" : "";
      const label =
        trendMetrics.trend === "improving"
          ? "改善傾向"
          : trendMetrics.trend === "declining"
            ? "悪化傾向"
            : "横ばい";
      historyLines.push(
        `→ トレンド分析: ${label} (${sign}${trendMetrics.deltaPct}%, ${trendMetrics.periodDays}日間, ${trendMetrics.sameSubjectOnly ? "同科目のみ" : "全科目平均"})`,
      );
      if (trendMetrics.fixedWeakUnits.length > 0) {
        historyLines.push(
          `→ 弱点固定化フラグ: ${trendMetrics.fixedWeakUnits.join("・")} (2回連続低下 — 根本原因を診断すること)`,
        );
      }
    }
  }
...
export async function diagnose(input: TestInput): Promise<Diagnosis> {
  ...
  // キャッシュチェック
  const cached = getCachedDiagnosis(input);
