...
- 1週間計画と「今日の45分ブロック」の具体タスクまでの落とし込み
を行うことです。

口調:
- 受験生の自尊心を潰さず、ただし甘やかさない。
- 「次の45分だけ勝てばいい」というメッセージを通底させる。
...
- 45分ブロックは「完了条件（completion）」を必ず行動可能な形で書く。例: 「4問中3問を答えを見ずに解ける」。
- weekPlanは7日分、todayBlocksは2〜3個。
`;
...
const JSON_SCHEMA_HINT = `
{
  ...
  "encouragement": "1-2文。励ましというよりは作戦の宣言。"
}
`;
...
function fallbackDiagnosis(input: TestInput): Diagnosis {
  ...
  return {
    ...
    encouragement: "次の45分だけ勝てばいい。それを積み上げれば届く。",
  };
}
