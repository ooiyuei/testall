...
export function TestListView() {
  ...
  const animatedCount = useCountUp(tests.length);
    ...
    <div className="px-5 pb-8 pt-2">
      ...
      <section className="flex items-end justify-between">
        <div>
          ...
          >
            <span className="text-sky-500 tabular-nums">{animatedCount}</span> 件の記録
          </h1>
        </div>
        <Link
          href="/app/test/new"
...
function TestRow({
  ...
  const relative = formatRelativeShort(test.createdAt);
  ...
  return (
    ...
    >
      ...
      <div className="w-12 flex-none text-center">
        ...
        <div className="mt-0.5 text-[9px] text-ink-400 truncate">{relative}</div>
      </div>
      {/* body */}
